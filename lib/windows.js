...
function windows() {
	...
	const windowTextLength = user32.GetWindowTextLengthW(activeWindowHandle) + 2;
	...
	const windowTextBuffer = Buffer.alloc(windowTextLength * wchar.size);
	// Write the window text to the buffer (it returns the text size, but it's not used here)
	user32.GetWindowTextW(activeWindowHandle, windowTextBuffer, windowTextLength);
	// The text as a JavaScript string
	const windowTitle = wchar.toString(windowTextBufferClean).replace(/\0/g, '');
	...
	const processPath = wchar.toString(processFileNameBuffer).replace(/\0/g, '');
